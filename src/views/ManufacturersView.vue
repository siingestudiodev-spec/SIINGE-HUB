        ...
        <div class="card-content">
          ...
          <div class="card-body">
            ...
            <div v-if="m.email_logs && m.email_logs.length > 0" class="email-history">
              <div v-for="(log, index) in m.email_logs" :key="index" class="reach-date" :class="{ 'overdue': index === m.email_logs.length - 1 && isOverdue(log.sentAt) }">
    ...
    <div v-if="certPopup.show" class="modal-overlay" @click.self="certPopup.show = false">
      <div class="modal max-w-400">
        ...
        <div class="cert-list-popup">
          <div v-for="c in certPopup.list" :key="c" class="cert-item"><span class="check-icon">✅</span> {{ c.trim() }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="emailModal.show" class="modal-overlay" @click.self="emailModal.show = false">
      <div class="modal">
        ...
        <div class="modal-field">
          ...
          <div class="signature-notice mt-2">✨ Your SIINGE STUDIO signature will be automatically attached at the bottom.</div>
        </div>
...
// Tu firma HTML comprimida en una sola línea para evitar cortes
const htmlSignature = `<br><br><table cellpadding="0" cellspacing="0" style="border-collapse:collapse;line-height:1.15;width:100%;" id="isPasted" width="100%"><tbody valign="middle"><tr valign="inherit"><td style="vertical-align:middle;padding:.01px 12px 0.01px 1px;width:92px;text-align:center;" valign="middle" align="center"><img border="0" src="https://permanent-assets-download.flockmail.com/signature/2408373/2024-06-03_36c3cd811224bc3a55b5_55761" width="92" alt="photo" style="width:78px;vertical-align:middle;border-radius:0px;height:83px;border:0px;display:block;"></td><td valign="top" style="padding:.01px 0.01px 0.01px 12px;vertical-align:top;border-left:solid 1px #BDBDBD;"><strong><strong><br></strong></strong><table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;" width="100%"><tbody valign="middle"><tr valign="inherit"><td style="padding:.01px;" valign="inherit"><p style="margin:.1px;line-height:108.0%;font-size:16px;"><span style="font-size:12pt;"><strong><strong>Luis Domínguez</strong><br style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:14px;font-weight:400;"></strong></span><span style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:11pt;font-weight:400;display:inline !important;"><strong>Product Operations Manager</strong></span></p><p style="margin:.1px;line-height:108.0%;font-size:16px;"><span style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:11pt;font-weight:400;display:inline !important;"><strong>​</strong></span><br style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:14px;font-weight:400;"><span style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:14px;font-weight:400;display:inline !important;">+57 350 201 4528 &nbsp; | &nbsp;&nbsp;</span><a href="https://www.siinge.studio/" style="color:rgb(76,140,246);font-family:Arial,sans-serif;font-size:14px;font-weight:400;" target="_blank">www.siinge.studio</a><br style="color:rgb(51,51,51);font-family:Arial,sans-serif;font-size:14px;font-weight:400;"><a href="mailto:production@siinge.studio" style="color:rgb(76,140,246);font-family:Arial,sans-serif;font-size:14px;font-weight:400;" target="_blank">production@siinge.studio</a></p></td></tr></tbody></table></td></tr></tbody></table><table cellpadding="0" cellspacing="0" width="100%" style="width:100%;color:gray;border-top:1px solid gray;line-height:normal;margin-top:10px;"><tbody valign="middle"><tr valign="inherit"><td style="padding:9px 8px 0 0;" valign="inherit"><p style="color:#888888;text-align:left;font-size:10px;margin:1px;line-height:120%;font-family:Arial;">IMPORTANT: The contents of this email and any attachments are confidential. They are intended for the named recipient(s) only. If you have received this email by mistake, please notify the sender immediately and do not disclose the contents to anyone or make copies thereof.</p></td></tr></tbody></table>`

const manufacturers = ref([]); const templatesList = ref([]); const loading = ref(true); const showForm = ref(false); const editing = ref(false); const editId = ref(null); const search = ref(''); const filterCountry = ref(''); const filterCategory = ref('');
...
const selectedCategories = ref([]); const selectedCertifications = ref([]); const certPopup = ref({ show: false, list: [] })
...
function isOverdue(dateString) { return !dateString ? false : Math.floor((new Date() - new Date(dateString)) / 86400000) >= 7 }

async function fetchManufacturers() {
  loading.value = true
  const { data } = await supabase.from('manufacturers').select('*').order('company_name')
  manufacturers.value = data || []; loading.value = false
}
...
function openEmailModal(m) { 
  emailModal.value = { show: true, to: m.email, subject: '', body: '', sending: false, manufacturerId: m.id, companyName: m.company_name, selectedTemplate: '', isInitialReach: false } 
}
...
function openInitialReachModal(m) {
  ...
  emailModal.value = { show: true, to: m.email, subject: 'Manufacturing Partnership Inquiry | SIINGE STUDIO', body: body, sending: false, manufacturerId: m.id, companyName: m.company_name, selectedTemplate: '', isInitialReach: true }
}
...
async function sendEmail() {
  ...
  try {
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { 
      to_email: emailModal.value.to, subject: emailModal.value.subject, message: emailModal.value.body, signature: htmlSignature 
    }, EMAILJS_PUBLIC_KEY)
    const sentAt = new Date().toISOString()
    ...
    fetchManufacturers(); emailModal.value.show = false
  } catch (err) { alert('Error sending email.'); console.error(err) } finally { emailModal.value.sending = false }
}

async function deleteManufacturer(id) {
  if (confirm('Delete this manufacturer?')) { await supabase.from('manufacturers').delete().eq('id', id); fetchManufacturers() }
}
...
input:focus, textarea:focus, select:focus { border-color: #6366f1; background: white; outline: none; }
...
.category-checkbox { display: flex; align-items: center; gap: 0.5rem; background: #f3f4f6; padding: 0.5rem 0.8rem; border-radius: 8px; cursor: pointer; font-size: 0.85rem; user-select: none; transition: all 0.2s; border: 1px solid transparent; }
...
.search-input { flex: 1; min-width: 200px; margin: 0; }
...
.card { background: white; border-radius: 16px; border: 1px solid #e5e7eb; display: flex; flex-direction: column; height: 100%; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); transition: transform 0.2s, box-shadow 0.2s; }
...
.card-title h3 { margin: 0 0 0.3rem 0; font-size: 1.1rem; font-weight: 700; color: #111827; }
...
/* BOTONES INFERIORES REDISEÑADOS Y ALINEADOS */
.card-actions { margin-top: auto; padding: 1.2rem 1.5rem; background: #f8fafc; border-top: 1px solid #e5e7eb; border-radius: 0 0 16px 16px; display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; }
.spacer { flex-grow: 1; }
.btn-action { display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.6rem 0.7rem; border-radius: 8px; font-size: 0.72rem; font-weight: 800; border: none; cursor: pointer; transition: all 0.2s; }
...
.btn-initial-reach:hover { background: #bae6fd; }
...
.modal { background: white; border-radius: 20px; width: 100%; max-width: 600px; padding: 2rem; max-height: 90vh; overflow-y: auto; }
...
/* GLOBAL BUTTONS */
.btn-primary { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; border: none; padding: 0.7rem 1.5rem; border-radius: 10px; cursor: pointer; font-weight: 700; font-size: 0.95rem; }
.btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-secondary { background: #f3f4f6; color: #4b5563; border: none; padding: 0.7rem 1.5rem; border-radius: 10px; cursor: pointer; font-weight: 700; }
.btn-clear { background: #f3f4f6; color: #6b7280; border: none; padding: 0.7rem 1rem; border-radius: 8px; cursor: pointer; font-weight: 700; margin: 0; }
