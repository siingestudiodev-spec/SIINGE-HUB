...
.form-card h2 { font-size: 1.1rem; margin-bottom: 1.25rem; color: #f8fafc; margin-top: 0;}
...
input:focus, textarea:focus, select:focus { outline: none; border-color: #6366f1; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.2); }
...
.btn-clear { background: #334155; color: #cbd5e1; border: 1px solid #475569; padding: 0.6rem 0.9rem; border-radius: 10px; cursor: pointer; font-size: 0.88rem; }
.btn-clear:hover { background: #475569; color: white;}
.results-count { color: #94a3b8; font-size: 0.85rem; white-space: nowrap; }
...
.card-title h3 { font-size: 1.05rem; font-weight: 700; color: #f8fafc; margin: 0 0 0.3rem 0; }
...
.status-closed { background: rgba(225, 29, 72, 0.2); color: #fb7185; border: 1px solid rgba(225, 29, 72, 0.3); }

/* CARD BODY */
.card-body { margin-bottom: 1.25rem; }
.info-row { display: flex; align-items: flex-start; gap: 0.5rem; margin-bottom: 0.45rem; font-size: 0.88rem; color: #cbd5e1; }
.info-icon { flex-shrink: 0; }
.date-row { color: #64748b !important; font-size: 0.82rem !important; }

/* CARD ACTIONS (Botones de colores) */
.card-actions { display: flex; gap: 0.5rem; padding-top: 1rem; border-top: 1px solid #334155; flex-wrap: wrap; align-items: center;}

/* Estilos oscuros para los botones de acción */
.btn-techpack { background: rgba(139, 92, 246, 0.15); color: #c4b5fd; border: 1px solid rgba(139, 92, 246, 0.3); padding: 0.5rem 0.9rem; border-radius: 8px; text-decoration: none; font-size: 0.85rem; font-weight: 600; }
.btn-techpack:hover { background: rgba(139, 92, 246, 0.25); }

.btn-quotes { background: rgba(34, 197, 94, 0.15); color: #86efac; border: 1px solid rgba(34, 197, 94, 0.3); padding: 0.5rem 0.9rem; border-radius: 8px; text-decoration: none; font-size: 0.85rem; font-weight: 600; }
.btn-quotes:hover { background: rgba(34, 197, 94, 0.25); }

.btn-sourcing { background: rgba(245, 158, 11, 0.15); color: #fcd34d; border: 1px solid rgba(245, 158, 11, 0.3); padding: 0.5rem 0.9rem; border-radius: 8px; text-decoration: none; font-size: 0.85rem; font-weight: 600; }
.btn-sourcing:hover { background: rgba(245, 158, 11, 0.25); }

.btn-timeline { background: rgba(236, 72, 153, 0.15); color: #f9a8d4; border: 1px solid rgba(236, 72, 153, 0.3); padding: 0.5rem 0.9rem; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; }
.btn-timeline:hover { background: rgba(236, 72, 153, 0.25); }

.btn-secondary { background: #334155; color: #e2e8f0; border: none; padding: 0.5rem 1rem; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; }
.btn-secondary:hover { background: #475569; }

.btn-danger { background: rgba(220, 38, 38, 0.15); color: #fca5a5; border: 1px solid rgba(220, 38, 38, 0.3); padding: 0.5rem 1rem; border-radius: 8px; cursor: pointer; font-size: 0.85rem; font-weight: 600; }
...
/* =========================================
   MODAL DE TIMELINE
========================================= */
.modal-overlay { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.75); display: flex; align-items: center; justify-content: center; z-index: 1000; padding: 1rem; backdrop-filter: blur(2px); }
.modal { background: #1e293b; padding: 2rem; border-radius: 16px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); border: 1px solid #334155; }
.modal-large { max-width: 950px; }
.modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #334155; padding-bottom: 1rem; margin-bottom: 1.5rem; }
.modal-header h2 { margin: 0; font-size: 1.25rem; color: #f8fafc; }
.header-actions { display: flex; gap: 1rem; align-items: center; }

.btn-reset-template { background: rgba(225, 29, 72, 0.1); color: #fb7185; border: 1px solid rgba(225, 29, 72, 0.3); padding: 0.4rem 0.8rem; border-radius: 8px; font-size: 0.8rem; font-weight: 600; cursor: pointer; }
.btn-reset-template:hover { background: rgba(225, 29, 72, 0.2); }
.modal-close { background: #334155; border: none; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; color: #cbd5e1; display: flex; align-items: center; justify-content: center; font-weight: bold;}
.modal-close:hover { background: #475569; color: white;}

.timeline-container { display: flex; flex-direction: column; gap: 0; border: 1px solid #334155; border-radius: 8px; overflow: hidden; }
.timeline-header-row { display: flex; padding: 0.75rem 1rem; background: #0f172a; font-size: 0.75rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; border-bottom: 1px solid #334155; }
.th-name { flex: 3; }
.th-date { flex: 1; text-align: center; }
.th-status { flex: 1; text-align: center; }
.th-action { width: 80px; }

.stage-group { border-bottom: 1px solid #334155; background: #1e293b; }
.stage-group:last-child { border-bottom: none; }

.expand-arrow { display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; color: #64748b; font-size: 0.75rem; transition: transform 0.2s; user-select: none; }
.small-arrow { font-size: 0.65rem; color: #475569; }
.empty-dot { color: #334155; }
.spacer-icon { width: 20px; display: inline-block; }

/* FILAS DE TIMELINE */
.task-row { display: flex; align-items: center; padding: 0.5rem 1rem; border-bottom: 1px solid #334155; cursor: pointer; transition: background 0.15s; }
.task-row:last-child { border-bottom: none; }

.level-1 { padding: 0.75rem 1rem; background: #1e293b; }
.level-1:hover { background: #334155; }
.level-2 { background: #0f172a; }
.level-2:hover { background: #1e293b; }
.level-3 { background: #0b1120; border-top: 1px dashed #334155; }
.level-3:hover { background: #1e293b; }

.task-info { flex: 3; display: flex; align-items: center; gap: 0.4rem; font-size: 0.9rem; color: #f8fafc; }
.indent-1 { padding-left: 1rem; }
.indent-2 { padding-left: 2.5rem; }
.level-1 strong { font-weight: 700; letter-spacing: 0.03em; }

.task-name-input { width: 100%; border: 1px solid transparent; background: transparent; padding: 0.2rem 0.4rem; font-size: 0.88rem; font-weight: 500; border-radius: 4px; color: #e2e8f0; transition: border 0.2s; margin-left: 0.2rem;}
.task-name-input:focus { border: 1px solid #6366f1; background: #1e293b; }

.task-empty-slots { flex: 2; }
.task-date { flex: 1; display: flex; justify-content: center; }
.date-input { width: 100%; max-width: 130px; padding: 0.3rem; font-size: 0.75rem; color: #e2e8f0; background: #0f172a; border: 1px solid #475569; border-radius: 6px; cursor: text;}

/* TIMELINE STATUS */
.task-status { flex: 1; display: flex; justify-content: center; }
.status-select { padding: 0.3rem 0.5rem; border-radius: 6px; font-size: 0.75rem; font-weight: 700; outline: none; cursor: pointer; width: 110px; background: #0f172a; border: 1px solid #475569; color: #e2e8f0; }

.status-pending { background: rgba(234, 179, 8, 0.15); color: #fde047; border-color: rgba(234, 179, 8, 0.5);}
.status-in-progress { background: rgba(14, 165, 233, 0.15); color: #7dd3fc; border-color: rgba(14, 165, 233, 0.5);}
.status-completed { background: rgba(34, 197, 94, 0.15); color: #86efac; border-color: rgba(34, 197, 94, 0.5);}

.notes-indicator { background: #ca8a04; color: #fef08a; font-size: 0.65rem; font-weight: 800; padding: 0.1rem 0.4rem; border-radius: 20px; margin-left: 0.3rem; }
.task-actions { width: 80px; display: flex; gap: 0.3rem; justify-content: flex-end; opacity: 0; transition: opacity 0.2s; }
.task-row:hover .task-actions { opacity: 1; }
.btn-note-micro, .btn-add-micro, .btn-del-micro { background: transparent; color: #94a3b8; border: none; font-size: 0.8rem; cursor: pointer; padding: 0.2rem; border-radius: 4px; }
.btn-note-micro:hover { background: #422006; color: #fde047; }
.btn-add-micro:hover { background: #082f49; color: #7dd3fc; }
.btn-del-micro:hover { background: #450a0a; color: #fca5a5; }
...
.notes-list { max-height: 300px; overflow-y: auto; margin-bottom: 1rem; border: 1px solid #334155; border-radius: 8px; padding: 0.5rem; background: #0f172a; }
.empty-notes { text-align: center; color: #64748b; font-size: 0.85rem; padding: 2rem; }
.note-item { background: #1e293b; padding: 0.75rem; border-radius: 8px; border: 1px solid #475569; margin-bottom: 0.5rem; }
.note-item:last-child { margin-bottom: 0; }
.note-meta { display: flex; justify-content: space-between; font-size: 0.75rem; color: #94a3b8; margin-bottom: 0.4rem; border-bottom: 1px dashed #334155; padding-bottom: 0.3rem;}
.note-meta strong { color: #818cf8; }
.note-text { font-size: 0.85rem; color: #e2e8f0; line-height: 1.5; white-space: pre-wrap; }
