  ...
  <div class="container">
    ...
    <div v-else class="cards-grid">
      <div v-for="item in projectMaterials" :key="item.id" class="card" :class="{ 'card-approved': item.is_approved }">
        ...
        <div class="card-body border-b pb-3 mb-3 bg-tracking" v-if="item.is_approved || item.ordered_date">
          <div class="text-xs uppercase text-green-600 font-bold mb-2">Order Tracking</div>
          <div class="info-row" v-if="item.approval_date"><strong>Approved:</strong> {{ formatDate(item.approval_date) }}</div>
          <div class="info-row" v-if="item.ordered_date"><strong>Ordered:</strong> {{ formatDate(item.ordered_date) }}</div>
          <div class="info-row" v-if="item.quantity_ordered"><strong>Qty Ordered:</strong> {{ item.quantity_ordered }}</div>
        </div>
      ...
      <div class="modal max-w-700">
        ...
        <div class="modal-body">
          ...
          <div v-if="form.sourcing_id" class="project-details-section">
            <label class="section-label">2. Technical & Tracking Data</label>
            <div class="form-grid">
              ...
              <div class="modal-field"><label>Approval Date</label><input type="date" v-model="form.approval_date" /></div>
              <div class="modal-field"><label>Ordered Date</label><input type="date" v-model="form.ordered_date" /></div>
              <div class="modal-field"><label>Quantity Ordered</label><input v-model="form.quantity_ordered" placeholder="e.g. 200 meters" /></div>
              ...
              <div class="modal-field"><label>Lead Time (Custom)</label><input v-model="form.custom_lead_time" /></div>
...
const form = ref({ 
  ...
  available_colors: '', color: '', price_per_meter: '', item_url: '', 
  project_notes: '', approval_date: '', ordered_date: '', quantity_ordered: ''
})
...
async function toggleApproval(item) {
  ...
  // Si se aprueba ahora, ponemos la fecha de hoy. Si se quita, la limpiamos.
  const approvalDate = newStatus ? new Date().toISOString().split('T')[0] : null
  
  const { error } = await supabase
    .from('project_materials')
    .update({ is_approved: newStatus, approval_date: approvalDate })
    .eq('id', item.id)

  if (!error) {
    item.is_approved = newStatus
    item.approval_date = approvalDate
  }
}
...
function openSourcingModal(item = null) { 
  if (item) {
    ...
    form.value = { sourcing_id: '', placement: '', specific_name: '', composition: '', custom_moq: '', custom_lead_time: '', instock_lead_time: '', available_colors: '', color: '', price_per_meter: '', item_url: '', project_notes: '', approval_date: '', ordered_date: '', quantity_ordered: '' }
  }
...
/* Sección de tracking en la tarjeta */
.bg-tracking { background-color: #f0fdf4; border-radius: 8px; padding: 0.75rem; margin-top: -0.5rem; margin-bottom: 1rem !important; border: 1px dashed #10b981; }
.text-green-600 { color: #059669; }

.factory-link-btn { background: transparent; border: none; padding: 0; color: #4f46e5; font-weight: 700; text-decoration: underline; cursor: pointer; font-size: 0.88rem; }
