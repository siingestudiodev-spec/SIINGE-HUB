      ...
      <div v-for="s in filtered" :key="s.id" class="row" :class="{ delivered: s.delivered_at }">
        <div class="col-main">
          <div class="row-top">
            ...
            <span v-if="s.delivered_at" class="delivered-badge">Delivered {{ formatDate(s.delivered_at) }}</span>
          </div>
          <div class="row-sub">
        ...
        <div class="col-actions">
          <button v-if="!s.delivered_at" @click="markDelivered(s)" class="btn-secondary">Delivered</button>
...
async function markDelivered(s) {
  ...
  const { error } = await supabase.from('project_shipments').update({ delivered_at: now }).eq('id', s.id)
...
.delivered-badge { font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #16a34a; background: rgba(34,197,94,0.12); padding: 2px 7px; border-radius: 10px; }
