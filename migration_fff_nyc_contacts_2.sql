-- Contacts from the business cards — Functional Fabric Fair NYC, July 2026
-- Run in Supabase SQL editor (Settings > SQL editor)
--
-- ⚠ CORRECT PROJECT: luqakyzgcgcafukfirfk (SIINGE HUB), NOT the CRM (pbylrmaqtmgnywzdliql).
--
-- Second batch. The companies were already loaded via the API; these contacts cannot be
-- inserted with the anon key because RLS on sourcing_contacts / manufacturer_contacts only
-- allows writes to the authenticated role.
--
-- Excludes the unclear cards Luis is reviewing by hand:
--   · Leah Galkin (WA0011) — the card carries no company name
--   · Ellie Lin (WA0012) — Donglong Lace and Qingyang Fabric logos, employer unconfirmed
--   · Jason / Koumtex (WA0014) — sticker stuck on the Dafabrics card
--
-- primary_contact_id is left NULL: Sierra stars the primaries.
...
FROM (VALUES
  -- New contacts for companies already in the hub
  ...
  -- Contacts for the 5 new Sourcing companies
...
FROM (VALUES
  -- Cheng Ye: Evan Hsu, General Manager of the US subsidiary
